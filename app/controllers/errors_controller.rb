class ErrorsController < ApplicationController
  ...
  # This code will be executed only once to bring one default error.
  def create_default_error
    error = Error.find_by_error_text("No answer selected. Select a correct answer.")
    if error.nil? or error.blank?
      error = Error.new
      error.error_text = "No answer selected. Select a correct answer."
      error.save
    end
  end
